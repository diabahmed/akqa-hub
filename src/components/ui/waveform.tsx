...
export const LiveMicrophoneWaveform = ({
  ...
  // eslint-disable-next-line react-hooks/immutability -- Resetting refs and state when toggling active is necessary for cleanup
  useEffect(() => {
    if (!active) {
    ...
    setDragOffset?.(0);
    historyRef.current = [];
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Necessary to reset state when component becomes inactive
    setHasHistory(false);
